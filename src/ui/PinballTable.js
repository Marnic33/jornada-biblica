/**
 * PinballTable — pinball 2D em Canvas, com física escrita à mão.
 * Mesa temática do dilúvio de Noé: bumpers-animais, rampa para a arca,
 * flippers na base. Visual moderno: gradientes, glow, partículas, chuva.
 *
 * Controles: esquerda/direita (teclado) ou tocar metades da tela (celular).
 * É autossuficiente — só precisa de um container e callbacks.
 */
export class PinballTable {
  constructor(root, audio, { onExit } = {}) {
    this.root = root;
    this.audio = audio;
    this.onExit = onExit;
    this.W = 480; this.H = 760; // resolução lógica da mesa
    this.score = 0;
    this.balls = 3;
    this.savedPairs = 0;
    this.running = false;
    this.particles = [];
    this.rain = [];
    this._build();
    this._initTable();
    this._bind();
  }

  _build() {
    this.el = document.createElement('div');
    this.el.className = 'screen pinball';
    this.el.innerHTML = `
      <div class="pinball-frame">
        <div class="pinball-hud">
          <div class="pb-stat"><span class="pb-label">Pontos</span><span id="pb-score">0</span></div>
          <div class="pb-stat center"><span class="pb-label">Pares salvos</span><span id="pb-pairs">0 / 5</span></div>
          <div class="pb-stat right"><span class="pb-label">Bolas</span><span id="pb-balls">3</span></div>
        </div>
        <canvas id="pb-canvas" width="${this.W}" height="${this.H}"></canvas>
        <div class="pinball-controls">
          <button class="pb-flip-btn" id="pb-left">◀ Esquerda</button>
          <button class="pb-exit" id="pb-exit">Sair</button>
          <button class="pb-flip-btn" id="pb-right">Direita ▶</button>
        </div>
        <div class="pinball-msg" id="pb-msg"></div>
      </div>
    `;
    this.root.appendChild(this.el);
    this.canvas = this.el.querySelector('#pb-canvas');
    this.ctx = this.canvas.getContext('2d');
    this.msg = this.el.querySelector('#pb-msg');
  }

  _initTable() {
    const W = this.W, H = this.H;
    // bola
    this.ball = { x: W - 30, y: H - 120, vx: 0, vy: 0, r: 10, onLauncher: true };
    this.gravity = 0.28;

    // flippers (dois, na base)
    const flY = H - 90;
    this.flippers = {
      left:  { x: W * 0.34, y: flY, len: 92, angle: 0.5, rest: 0.5, up: -0.5, dir: 1, active: false },
      right: { x: W * 0.66, y: flY, len: 92, angle: Math.PI - 0.5, rest: Math.PI - 0.5, up: Math.PI + 0.5, dir: -1, active: false },
    };

    // paredes inclinadas que afunilam para os flippers
    this.walls = [
      { x1: 20, y1: H - 230, x2: W * 0.34 - 30, y2: flY + 6 },         // funil esquerdo
      { x1: W - 20, y1: H - 230, x2: W * 0.66 + 30, y2: flY + 6 },     // funil direito
    ];

    // bumpers em forma de animais (acertar acende = par salvo)
    const animals = ['🦁','🐘','🐑','🐫','🦌'];
    this.bumpers = animals.map((emoji, i) => ({
      x: 70 + (i % 3) * 170 + (i >= 3 ? 85 : 0),
      y: 180 + Math.floor(i / 3) * 130,
      r: 32, emoji, lit: false, hitFlash: 0,
    }));

    // alvo da arca (topo central) — encestar dá bônus
    this.arkGate = { x: W / 2, y: 80, w: 120, h: 44 };

    // chuva decorativa
    for (let i = 0; i < 60; i++) {
      this.rain.push({ x: Math.random() * W, y: Math.random() * H, v: 4 + Math.random() * 4, len: 8 + Math.random() * 8 });
    }
  }

  _bind() {
    this._keydown = (e) => {
      if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'a') this.flippers.left.active = true;
      if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'd') this.flippers.right.active = true;
      if (e.key === ' ' && this.ball.onLauncher) this._launch();
    };
    this._keyup = (e) => {
      if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'a') this.flippers.left.active = false;
      if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'd') this.flippers.right.active = false;
    };
    window.addEventListener('keydown', this._keydown);
    window.addEventListener('keyup', this._keyup);

    const L = this.el.querySelector('#pb-left'), R = this.el.querySelector('#pb-right');
    const press = (fl, v) => (e) => { e.preventDefault(); this.flippers[fl].active = v; if (v && this.ball.onLauncher) this._launch(); };
    L.addEventListener('touchstart', press('left', true), { passive: false });
    L.addEventListener('touchend', press('left', false));
    L.addEventListener('mousedown', press('left', true));
    L.addEventListener('mouseup', press('left', false));
    R.addEventListener('touchstart', press('right', true), { passive: false });
    R.addEventListener('touchend', press('right', false));
    R.addEventListener('mousedown', press('right', true));
    R.addEventListener('mouseup', press('right', false));
    // tocar a própria mesa também aciona (metade esquerda/direita)
    this.canvas.addEventListener('touchstart', (e) => {
      e.preventDefault();
      const rect = this.canvas.getBoundingClientRect();
      for (const t of e.touches) {
        const side = (t.clientX - rect.left) < rect.width / 2 ? 'left' : 'right';
        this.flippers[side].active = true;
      }
      if (this.ball.onLauncher) this._launch();
    }, { passive: false });
    this.canvas.addEventListener('touchend', () => { this.flippers.left.active = false; this.flippers.right.active = false; });

    this.el.querySelector('#pb-exit').onclick = () => { this.stop(); this.onExit?.(); };
    this._showMsg('Toque ou ESPAÇO para lançar a bola', 0);
  }

  _launch() {
    if (!this.ball.onLauncher) return;
    this.ball.onLauncher = false;
    this.ball.vx = -2 - Math.random() * 1.5;
    this.ball.vy = -16;
    this._showMsg('', 0);
  }

  start() {
    this.running = true;
    this.last = performance.now();
    const loop = (now) => {
      if (!this.running) return;
      const dt = Math.min((now - this.last) / 16.67, 2);
      this.last = now;
      this._update(dt);
      this._draw();
      this._raf = requestAnimationFrame(loop);
    };
    this._raf = requestAnimationFrame(loop);
  }

  stop() {
    this.running = false;
    cancelAnimationFrame(this._raf);
    window.removeEventListener('keydown', this._keydown);
    window.removeEventListener('keyup', this._keyup);
  }

  _update(dt) {
    const b = this.ball, W = this.W, H = this.H;

    // animação dos flippers
    for (const key of ['left', 'right']) {
      const f = this.flippers[key];
      const target = f.active ? f.up : f.rest;
      f.angle += (target - f.angle) * 0.4 * dt;
    }

    // chuva
    for (const r of this.rain) {
      r.y += r.v * dt; if (r.y > H) { r.y = -10; r.x = Math.random() * W; }
    }
    // partículas
    for (let i = this.particles.length - 1; i >= 0; i--) {
      const p = this.particles[i];
      p.x += p.vx * dt; p.y += p.vy * dt; p.vy += 0.2 * dt; p.life -= dt;
      if (p.life <= 0) this.particles.splice(i, 1);
    }

    if (b.onLauncher) return;

    // física da bola
    b.vy += this.gravity * dt;
    b.x += b.vx * dt;
    b.y += b.vy * dt;

    // paredes laterais
    if (b.x < b.r) { b.x = b.r; b.vx *= -0.7; }
    if (b.x > W - b.r) { b.x = W - b.r; b.vx *= -0.7; }
    if (b.y < b.r) { b.y = b.r; b.vy *= -0.7; }

    // teto / topo: zona da arca dá bônus
    if (b.y < this.arkGate.y + 30 && Math.abs(b.x - this.arkGate.x) < this.arkGate.w / 2 && b.vy < 0) {
      this._addScore(500);
      this._showMsg('🛟 +500 Arca!', 60);
      this._burst(b.x, b.y, '#e8c878');
      b.vy = Math.abs(b.vy) * 0.6;
    }

    // bumpers-animais
    for (const bm of this.bumpers) {
      const dx = b.x - bm.x, dy = b.y - bm.y;
      const d = Math.hypot(dx, dy);
      if (d < b.r + bm.r) {
        const nx = dx / d, ny = dy / d;
        // empurra a bola para fora
        b.x = bm.x + nx * (b.r + bm.r);
        b.y = bm.y + ny * (b.r + bm.r);
        const dot = b.vx * nx + b.vy * ny;
        b.vx -= 2 * dot * nx; b.vy -= 2 * dot * ny;
        b.vx *= 1.05; b.vy *= 1.05; // bumper acelera (energia)
        bm.hitFlash = 12;
        this._burst(bm.x, bm.y, '#7fd1ff');
        this.audio?.collect?.();
        if (!bm.lit) { bm.lit = true; this.savedPairs++; this._addScore(300); this._updatePairs(); this._checkWin(); }
        else this._addScore(100);
      }
      if (bm.hitFlash > 0) bm.hitFlash -= dt;
    }

    // funis (paredes inclinadas) — colisão de segmento
    for (const w of this.walls) this._segmentBounce(b, w);

    // flippers — colisão
    this._flipperBounce(this.flippers.left, b, +1);
    this._flipperBounce(this.flippers.right, b, -1);

    // perdeu a bola (caiu embaixo)
    if (b.y > H + 30) this._loseBall();
  }

  _segmentBounce(b, w) {
    const { x1, y1, x2, y2 } = w;
    const dx = x2 - x1, dy = y2 - y1;
    const len2 = dx * dx + dy * dy;
    let t = ((b.x - x1) * dx + (b.y - y1) * dy) / len2;
    t = Math.max(0, Math.min(1, t));
    const px = x1 + t * dx, py = y1 + t * dy;
    const ddx = b.x - px, ddy = b.y - py;
    const d = Math.hypot(ddx, ddy);
    if (d < b.r) {
      const nx = ddx / (d || 1), ny = ddy / (d || 1);
      b.x = px + nx * b.r; b.y = py + ny * b.r;
      const dot = b.vx * nx + b.vy * ny;
      b.vx -= 2 * dot * nx; b.vy -= 2 * dot * ny;
      b.vx *= 0.9; b.vy *= 0.9;
    }
  }

  _flipperBounce(f, b, side) {
    // representa o flipper como um segmento da base (x,y) até a ponta
    const ex = f.x + Math.cos(f.angle) * f.len;
    const ey = f.y + Math.sin(f.angle) * f.len;
    const seg = { x1: f.x, y1: f.y, x2: ex, y2: ey };
    const dx = seg.x2 - seg.x1, dy = seg.y2 - seg.y1;
    const len2 = dx * dx + dy * dy;
    let t = ((b.x - seg.x1) * dx + (b.y - seg.y1) * dy) / len2;
    t = Math.max(0, Math.min(1, t));
    const px = seg.x1 + t * dx, py = seg.y1 + t * dy;
    const ddx = b.x - px, ddy = b.y - py;
    const d = Math.hypot(ddx, ddy);
    if (d < b.r + 6) {
      const nx = ddx / (d || 1), ny = ddy / (d || 1);
      b.x = px + nx * (b.r + 6); b.y = py + ny * (b.r + 6);
      const dot = b.vx * nx + b.vy * ny;
      b.vx -= 2 * dot * nx; b.vy -= 2 * dot * ny;
      // impulso extra se o flipper está ativo (rebatendo)
      if (f.active) { b.vy -= 9; b.vx += side * 3; this.audio?.collect?.(); }
      b.vx *= 0.96; b.vy *= 0.96;
    }
  }

  _loseBall() {
    this.balls--;
    this._updateBalls();
    if (this.balls <= 0) {
      this._gameOver();
    } else {
      this.ball.onLauncher = true;
      this.ball.x = this.W - 30; this.ball.y = this.H - 120;
      this.ball.vx = 0; this.ball.vy = 0;
      this._showMsg('Bola perdida! Toque para lançar', 0);
    }
  }

  _checkWin() {
    if (this.savedPairs >= this.bumpers.length) {
      this._addScore(2000);
      setTimeout(() => this._gameOver(true), 800);
    }
  }

  _gameOver(win = false) {
    this.running = false;
    cancelAnimationFrame(this._raf);
    this.msg.innerHTML = `
      <div class="pb-over">
        <h2>${win ? '🌈 Todos a salvo!' : 'Fim de jogo'}</h2>
        <p>Pontuação: <b>${this.score}</b></p>
        <button class="btn" id="pb-again">Jogar de novo</button>
        <button class="btn btn-ghost" id="pb-back">Voltar</button>
      </div>`;
    this.msg.classList.add('show');
    this.msg.querySelector('#pb-again').onclick = () => { this.msg.classList.remove('show'); this._reset(); };
    this.msg.querySelector('#pb-back').onclick = () => { this.stop(); this.onExit?.(); };
  }

  _reset() {
    this.score = 0; this.balls = 3; this.savedPairs = 0; this.particles = [];
    this.bumpers.forEach(b => { b.lit = false; b.hitFlash = 0; });
    this._updateScore(); this._updatePairs(); this._updateBalls();
    this.ball.onLauncher = true;
    this.ball.x = this.W - 30; this.ball.y = this.H - 120; this.ball.vx = 0; this.ball.vy = 0;
    this.running = true; this.last = performance.now();
    this._showMsg('Toque para lançar a bola', 0);
    const loop = (now) => {
      if (!this.running) return;
      const dt = Math.min((now - this.last) / 16.67, 2); this.last = now;
      this._update(dt); this._draw(); this._raf = requestAnimationFrame(loop);
    };
    this._raf = requestAnimationFrame(loop);
  }

  _addScore(n) { this.score += n; this._updateScore(); }
  _updateScore() { this.el.querySelector('#pb-score').textContent = this.score; }
  _updatePairs() { this.el.querySelector('#pb-pairs').textContent = `${this.savedPairs} / ${this.bumpers.length}`; }
  _updateBalls() { this.el.querySelector('#pb-balls').textContent = this.balls; }
  _showMsg(t, frames) { this.msg.textContent = t; this.msg.classList.toggle('show', !!t); }

  _burst(x, y, color) {
    for (let i = 0; i < 12; i++) {
      const a = Math.random() * Math.PI * 2, s = 1 + Math.random() * 4;
      this.particles.push({ x, y, vx: Math.cos(a) * s, vy: Math.sin(a) * s, life: 20 + Math.random() * 15, color });
    }
  }

  _draw() {
    const ctx = this.ctx, W = this.W, H = this.H;
    // fundo tempestuoso (gradiente)
    const g = ctx.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, '#1e2a3a'); g.addColorStop(0.5, '#2a3d52'); g.addColorStop(1, '#1a2330');
    ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);

    // chuva
    ctx.strokeStyle = 'rgba(150,180,210,0.35)'; ctx.lineWidth = 1.5;
    for (const r of this.rain) { ctx.beginPath(); ctx.moveTo(r.x, r.y); ctx.lineTo(r.x - 2, r.y + r.len); ctx.stroke(); }

    // arca (alvo topo)
    const ag = this.arkGate;
    ctx.save();
    ctx.shadowColor = '#e8c878'; ctx.shadowBlur = 20;
    ctx.fillStyle = '#7a4f2a';
    this._roundRect(ag.x - ag.w / 2, ag.y - ag.h / 2, ag.w, ag.h, 10); ctx.fill();
    ctx.shadowBlur = 0;
    ctx.font = '30px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('🛟', ag.x, ag.y);
    ctx.restore();

    // funis
    ctx.strokeStyle = 'rgba(212,168,83,0.6)'; ctx.lineWidth = 5; ctx.lineCap = 'round';
    for (const w of this.walls) { ctx.beginPath(); ctx.moveTo(w.x1, w.y1); ctx.lineTo(w.x2, w.y2); ctx.stroke(); }

    // bumpers-animais
    for (const bm of this.bumpers) {
      ctx.save();
      ctx.shadowColor = bm.lit ? '#7fd17f' : '#7fb0ff';
      ctx.shadowBlur = bm.hitFlash > 0 ? 30 : (bm.lit ? 18 : 10);
      ctx.fillStyle = bm.lit ? 'rgba(127,209,127,0.25)' : 'rgba(40,60,85,0.85)';
      ctx.beginPath(); ctx.arc(bm.x, bm.y, bm.r, 0, Math.PI * 2); ctx.fill();
      ctx.lineWidth = 3; ctx.strokeStyle = bm.lit ? '#7fd17f' : '#7fb0ff'; ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.font = '34px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText(bm.emoji, bm.x, bm.y + 1);
      if (bm.lit) { ctx.font = '16px serif'; ctx.fillText('✓', bm.x + bm.r - 4, bm.y - bm.r + 6); }
      ctx.restore();
    }

    // flippers
    for (const key of ['left', 'right']) {
      const f = this.flippers[key];
      const ex = f.x + Math.cos(f.angle) * f.len, ey = f.y + Math.sin(f.angle) * f.len;
      ctx.save();
      ctx.strokeStyle = '#e8c878'; ctx.lineWidth = 16; ctx.lineCap = 'round';
      ctx.shadowColor = '#cf9f43'; ctx.shadowBlur = 12;
      ctx.beginPath(); ctx.moveTo(f.x, f.y); ctx.lineTo(ex, ey); ctx.stroke();
      ctx.restore();
    }

    // partículas
    for (const p of this.particles) {
      ctx.globalAlpha = Math.max(0, p.life / 30);
      ctx.fillStyle = p.color;
      ctx.beginPath(); ctx.arc(p.x, p.y, 3, 0, Math.PI * 2); ctx.fill();
    }
    ctx.globalAlpha = 1;

    // bola
    const b = this.ball;
    ctx.save();
    ctx.shadowColor = '#fff'; ctx.shadowBlur = 12;
    const bg = ctx.createRadialGradient(b.x - 3, b.y - 3, 2, b.x, b.y, b.r);
    bg.addColorStop(0, '#ffffff'); bg.addColorStop(1, '#9aa8b8');
    ctx.fillStyle = bg;
    ctx.beginPath(); ctx.arc(b.x, b.y, b.r, 0, Math.PI * 2); ctx.fill();
    ctx.restore();

    // calha do lançador (direita)
    ctx.strokeStyle = 'rgba(212,168,83,0.4)'; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(W - 18, H - 230); ctx.lineTo(W - 18, H - 60); ctx.stroke();
  }

  _roundRect(x, y, w, h, r) {
    const ctx = this.ctx;
    ctx.beginPath();
    ctx.moveTo(x + r, y);
    ctx.arcTo(x + w, y, x + w, y + h, r);
    ctx.arcTo(x + w, y + h, x, y + h, r);
    ctx.arcTo(x, y + h, x, y, r);
    ctx.arcTo(x, y, x + w, y, r);
    ctx.closePath();
  }

  show() { this.el.classList.remove('gone'); requestAnimationFrame(() => this.el.classList.remove('hidden')); }
  hide() { this.el.classList.add('hidden'); setTimeout(() => this.el.classList.add('gone'), 400); }
  dispose() { this.stop(); this.canvas?.remove(); this.el?.remove(); }
}
